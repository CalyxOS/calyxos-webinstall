...
export async function verify(signature: Signature, originalContent: Uint8Array | Buffer | String | ReadableStream, publicKey: PublicKey): Promise<boolean> {
...
export default class MinisignVerify {
  static async verify(release: { url: string }, content: Uint8Array) {
    const signatureFile = await fetchSignature(release.url)
