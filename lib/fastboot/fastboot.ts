...
export class FastbootDevice {
  ...
  async connect() {
    ...
    if (devices.length === 1) {
      ...
      common.logDebug("No or multiple paired devices are connected, requesting one")
  ...
  async getVariable(varName: string): Promise<string | null> {
    let resp
    try {
      resp = (await common.runWithTimeout(this.runCommand(`getvar:${varName}`), GETVAR_TIMEOUT))
        .text
    } catch (error) {
   ...
   * @throws {FastbootError}
   */
  async bootBlob(blob: Blob, onProgress: FlashProgressCallback = (_progress) => {}) {
