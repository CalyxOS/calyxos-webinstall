  ...
  <div>
    ...
    >
      <template v-slot:[`item.1`]>
        <prepare-step :device="device" :active="curStep === 1" />
      </template>

      <template v-slot:[`item.2`]>
        <connect-step :device="device" :active="curStep === 2" />
      </template>

      <template v-slot:[`item.3`]>
        <unlock-step :device="device" :curStep="curStep" :stepNum="3" />
      </template>
      ...
      <template v-slot:[`item.6`]>
        <lock-step :device="device" :curStep="curStep" :stepNum="6" />
      </template>

      <template v-slot:[`item.7`]>
        <finish-step :device="device" :active="curStep === 7" />
      </template>
    </v-stepper>
    ...
    <v-dialog v-model="connectSelectDialog" :width="userAgent.includes('Windows') ? 600 : 500">
      <v-card>
        ...
        <v-card-text>
          <p>You need to select a device to continue installing.</p>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="primary" text @click="retryConnectSelect"> Retry </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="connectUdevDialog" width="500" persistent>
      <v-card>
        ...
        <v-card-text>
          <p>On Linux, users aren’t allowed to access USB devices by default.</p>
          <p>To fix this, install Android udev rules on your system:</p>

          <v-list>
            <v-list-item
              title="Arch Linux"
              subtitle="sudo pacman -S
                                    android-udev"
              lines="two"
            >
            </v-list-item>
          ...
          <p>
            Once you’ve installed udev rules, unplug your device and plug it back in for it to take
            effect.
          </p>
        </v-card-text>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="primary" text @click="retryConnectUdev"> Retry </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="claimDialog" width="500" persistent>
      <v-card>
        <v-card-title class="headline"> Can’t control device </v-card-title>

        <v-card-text>
          <p>Another app is taking control of your device, so we can’t talk to it.</p>
          <p>This is usually caused by having this installer open in another browser tab.</p>
          <p>
            To fix this, close all other web installer tabs in your browser and make sure you don’t
            have any tabs remaining in other windows.
          </p>
        </v-card-text>

        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="primary" text @click="retryClaim"> Retry </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="disconnectDialog" width="500" persistent>
      <v-card>
        <v-card-title class="headline"> Device disconnected </v-card-title>

        <v-card-text>
          <p>Your device disconnected, so we can’t continue installing.</p>
          <p>To fix this:</p>
          <ul class="ml-4 mb-4">
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="primary" text @click="retryDisconnect"> Retry </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="storageDialog" width="500" persistent>
      <v-card>
        ...
        <v-card-text>
          <p>
            There isn’t enough storage space available to download and unpack the OS. You need at
            least 5 GB free.
          </p>
          <p>
            If you’re not low on storage, this is caused by using an incognito window or guest
            browser profile. These profiles have very low storage limits, so installing from them
            isn’t possible.
          </p>
          <p>
            To fix this,
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="primary" text @click="retryStorage"> Retry </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="memoryDialog" width="500" persistent>
      <v-card>
        ...
        <v-card-text>
          <p>There isn’t enough free memory (RAM) to unpack and install the OS.</p>
          <p>
            To fix this,
            <strong>close some unused apps</strong> and try again.
          </p>
          <p>
            If it still doesn’t work, you may need to install from another computer or device with
            more memory.
          </p>
        </v-card-text>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="primary" text @click="retryMemory"> Retry </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="reconnectDialog" width="500" persistent>
      <v-card>
        ...
        <v-card-text>
          ...
          <v-banner single-line outlined rounded class="mt-8" v-if="reconnectError">
            <v-icon color="red darken-3">mdi-close</v-icon>
            <div class="my-4">
              <span class="text-body-1 red--text text--darken-3">{{ reconnectError }}</span>
            </div>
          </v-banner>
        </v-card-text>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="primary" text @click="requestReconnect"> Reconnect </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="timeoutDialog" width="500" persistent>
      <v-card>
        ...
        <v-card-text>
          <p>The connection to your device is stuck.</p>
          <p>
            To fix this, use the volume buttons to highlight
            <strong>“Restart bootloader”</strong> in the bootloader menu, and press the power
            button to select it.
          </p>
        </v-card-text>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="primary" text @click="retryTimeout"> Retry </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </div>
    ...
    emitError(err, retryCallback = undefined) {
      ...
      if (errEvent !== null) {
        if ("handleSelfError" in this) {
          this.handleSelfError(`error${errEvent}`, retryCallback || this.errorRetry)
        }

        this.emit(`error${errEvent}`, retryCallback || this.errorRetry)
      }
